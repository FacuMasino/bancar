<%@page import="domainModel.Client"%> <%@page import="java.util.List"%> <%@page
import="java.util.ArrayList"%> <%@taglib prefix="t" tagdir="/WEB-INF/tags/"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%> <%-- Variables
JSTL --%>
    ...
    <div class="container flex flex-col gap-4 mx-auto p-4 max-w-7xl mb-8">
      ...
      <div class="flex flex-col p-6 bg-white rounded-lg drop-shadow-sm">
        ...
        <div class="flex flex-col w-fit">
          ...
          <div class="flex justify-between gap-2 text-red-600">
            <span class="font-bold">Deuda:(no va por enunciado....)</span>
            <span>$ 10.000</span>
          </div>
      ...
      <c:if test="${client.accounts.size() < 3}">
        ...
        >
          ...
          <button type="submit" name="action" value="newAccount" class="btn btn-primary w-fit">Crear cuenta</button>
        </form>
      </c:if>
      ...
      <dialog id="modal_delete_account" class="modal">
        <form method="post" action="Accounts" class="modal-box bg-white">
          ...
          <p class="pt-4 pb-1">Est&aacute; seguro que desea eliminar la cuenta? </p>
          <p class="py-1 text-red-600">Esta acci&oacute;n no se puede deshacer!</p>
          <div class="modal-action">
            <button
              type="button"
              onclick="modal_delete_account.close()"
              class="btn"
            >
              Cancelar
            </button>
            <button type="submit" name="action" value="deleteAccount" class="btn btn-primary">Eliminar</button>
          </div>
        </form>
      </dialog>
      ...
      <dialog id="modal_edit_account" class="modal">
        <form method="post" action="Accounts" class="modal-box bg-white">
          ...
          <div class="py-4 flex items-center justify-end gap-2.5">
            <span>Saldo:</span>
            <input id="accountNumberEdit" type="hidden" name="accountId" />
            <input
              id="balanceInput"
              type="number"
              name="accountBalance"
          ...
          <div class="modal-action">
            ...
            <button
              type="button"
              onclick="modal_edit_account.close()"
              class="btn"
            >
              Cancelar
            </button>
            <button type="submit" name="action" value="editAccount" class="btn btn-primary">Modificar</button>
          </div>
        </form>
      </dialog>
    </div>
